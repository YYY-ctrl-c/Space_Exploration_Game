    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DB.getConnection()) {
            ...
            PreparedStatement psCheck = conn.prepareStatement(
                    "SELECT coins FROM users WHERE id = ? AND coins >= ? FOR UPDATE"
            );
            ...
            if (rewardRs.next()) {
                ...
                if ("coin".equals(rewardType)) {
                    ...
                    conn.prepareStatement(
                            "UPDATE users SET coins = coins + " + randomVal + " WHERE id = " + userId
                    ).executeUpdate();

                } else if ("crew".equals(rewardType)) {
                    rewardId = randomVal;
                    // 获取舰员基础信息
                    PreparedStatement basePs = conn.prepareStatement(
                            "SELECT name, fatigue_max FROM crew_base WHERE id = ?"
                    );
                    ...
                    int baseFatigue = 100;
                    if (baseRs.next()) {
                        crewName = baseRs.getString("name");
                        baseFatigue = baseRs.getInt("fatigue_max");
                    }
                    // 插入 user_crew（舰员可能重复，但 user_crew 无唯一约束，允许重复获得同一舰员）
                    PreparedStatement crewPs = conn.prepareStatement(
                            "INSERT INTO user_crew (user_id, crew_id, nickname, fatigue_max) VALUES (?, ?, ?, ?)"
                    );
                    crewPs.setInt(1, userId);
                    crewPs.setInt(2, rewardId);
                    crewPs.setString(3, crewName);
                    crewPs.setInt(4, baseFatigue);
                    crewPs.executeUpdate();
                    finalDisplayName = crewName;

                } else if ("supply".equals(rewardType)) {
                    rewardId = randomVal;
                    // 查询物资名字
                    PreparedStatement namePs = conn.prepareStatement(
                            "SELECT name FROM supply_base WHERE id = ?"
                    );
                    namePs.setInt(1, rewardId);
                    ResultSet nameRs = namePs.executeQuery();
                    if (nameRs.next()) finalDisplayName = nameRs.getString("name");

                    // 【关键修改】先查询是否已存在该物品，避免重复插入多行
                    PreparedStatement checkItemPs = conn.prepareStatement(
                            "SELECT amount FROM user_items WHERE user_id = ? AND item_id = ?"
                    );
                    checkItemPs.setInt(1, userId);
                    checkItemPs.setInt(2, rewardId);
                ...
                PreparedStatement logPs = conn.prepareStatement(
                        "INSERT INTO gacha_logs (user_id, gacha_type, reward_type, reward_id, reward_amount, reward_name, rarity) " +
                                "VALUES (?, ?, ?, ?, ?, ?, ?)"
                );
                logPs.setInt(1, userId);
                logPs.setString(2, gachaType);
                logPs.setString(3, rewardType);
                if (rewardId != null) logPs.setInt(4, rewardId);
                else logPs.setNull(4, Types.INTEGER);
                if (rewardAmount != null) logPs.setInt(5, rewardAmount);
                else logPs.setNull(5, Types.INTEGER);
                logPs.setString(6, finalDisplayName);
                logPs.setString(7, rarity);
                logPs.executeUpdate();

                // 获取最新余额
                ResultSet coinsRs = conn.createStatement().executeQuery(
